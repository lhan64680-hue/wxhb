...
try:
    ...
    # ComfyUI 0.30 (the pinned H3 runtime used by Infinite Canvas) predates
    # the runtime-bypass adapter module.  The plugin remains usable there via
    # the built-in merged LoRA patch path, which is also the low-VRAM mode.
    _HAS_WEIGHT_ADAPTER = False
else:
    _HAS_WEIGHT_ADAPTER = True
...
def _apply_bypass_lora(new_model, lora, modules, strength):
    ...
    if not _HAS_WEIGHT_ADAPTER:
        # Compatibility fallback for the bundled ComfyUI 0.30 runtime.  The
        # official low-VRAM merge path is deterministic and keeps peak memory
        # suitable for the local 16 GB profile.
        return len(new_model.add_patches(comfy.lora.load_lora(lora, key_map, log_missing=False), strength))
...
class MiniMaxH3TurboLoRA:
    ...
    def apply_lora(self, model, lora_name, strength, low_vram=False):
        ...
        mode = "merge" if low_vram or not _HAS_WEIGHT_ADAPTER else "bypass"
        ...
        n_fc2 = 0
        if low_vram or not _HAS_WEIGHT_ADAPTER:
