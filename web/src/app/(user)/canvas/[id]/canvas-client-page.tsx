...
function NodeCreateMenu({ position, onCreate, onUpload, onOpenAssetLibrary, onClose }: { position: Position; onCreate: (type: CanvasNodeType) => void; onUpload: () => void; onOpenAssetLibrary: () => void; onClose: () => void }) {
    ...
    return (
        ...
        >
            ...
            <p className="px-1 pb-1 pt-2 text-xs leading-5" style={{ color: theme.node.muted }}>
                创建后拖动节点两侧连接点，即可建立上下游关系。
            </p>
        </div>
    );
}
        ...
        (type: CanvasNodeType, position?: Position, textContent?: string, nodeId?: string) => {
            ...
            const configMetadata =
                type === CanvasNodeType.Config
                    ...
                    : undefined;
            const textMetadata = type === CanvasNodeType.Text ? { model: KIMI_K3_MODEL, channelId: KIMI_K3_CHANNEL_ID, textCreativeMode: "write" as const } : undefined;
            const newNode = createCanvasNode(type, targetPosition, type === CanvasNodeType.Text && textContent !== undefined ? { ...textMetadata, content: textContent, status: NODE_STATUS_SUCCESS } : textMetadata || configMetadata, nodeId);
