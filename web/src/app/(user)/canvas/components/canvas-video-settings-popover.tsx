...
import { supportsMiniMaxH3FullReferences, supportsVideoFrameReferences } from "@/lib/video-model-capabilities";
    ...
    const panel =
        open && buttonRect ? (
            <VideoSettingsPortal
                ...
                metadata={metadata}
                firstFrameNodeId={firstFrameNodeId}
                lastFrameNodeId={lastFrameNodeId}
...
function VideoSettingsPortal({
    ...
    firstFrameNodeId?: string;
    lastFrameNodeId?: string;
    onFrameChange?: CanvasVideoSettingsPopoverProps["onFrameChange"];
    ...
    const h3FullReferenceCapable = !isKlingV3 && supportsMiniMaxH3FullReferences(model);
        ...
        <div ref={panelRef} className="canvas-image-settings-popover" style={style} onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
            <div className="space-y-4">
                ...
                {!visualOnly && isKlingV3 ? (
                    <KlingV3AdvancedSettings config={config} metadata={metadata} resourceOptions={resourceOptions} theme={theme} isKIEKlingV3={isKIEKlingV3} onConfigChange={onConfigChange} onMetadataChange={onMetadataChange} />
                ) : null}
                {!visualOnly && h3FullReferenceCapable ? <H3ReferenceSettings metadata={metadata} resourceOptions={resourceOptions} theme={theme} onMetadataChange={onMetadataChange} /> : null}
                {!visualOnly && frameReferencesEnabled && !h3FullReferenceCapable ? (
                ...
                <CanvasSettingGroup title="全能参考素材" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                        <p className="text-xs leading-5 opacity-65">
                            关联图片、视频和音频节点后在此选择。未选时会使用全部关联素材；最多 9 张图片、3 个视频、3 段音频（总计 12 项）。提示词可用 &lt;Picture 1&gt;、&lt;Video 1&gt;、&lt;Audio 1&gt; 指定用途。
                        </p>
                        <MultiResourcePicker
                            values={selectedIds}
                            options={resourceOptions.filter((item) => item.kind === "image" || item.kind === "video" || item.kind === "audio")}
                            theme={theme}
                            max={12}
                            onChange={(nodeIds) => onMetadataChange?.({ h3ReferenceNodeIds: nodeIds })}
                        />
                    </div>
                </CanvasSettingGroup>
            ) : (
                <CanvasSettingGroup title="首尾帧" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                        <FrameReferencePicker
                            label="首帧"
                            value={metadata?.firstFrameNodeId || ""}
                            options={resourceOptions.filter((item) => item.kind === "image")}
                            theme={theme}
                            onChange={(value) => onMetadataChange?.({ firstFrameNodeId: value || undefined })}
                        />
                        <FrameReferencePicker
                            label="尾帧"
                            value={metadata?.lastFrameNodeId || ""}
                            options={resourceOptions.filter((item) => item.kind === "image")}
                            theme={theme}
                            onChange={(value) => onMetadataChange?.({ lastFrameNodeId: value || undefined })}
                        />
                    </div>
                </CanvasSettingGroup>
            )}
            ...
            <CanvasSettingGroup title="多镜头分镜" color={theme.node.muted}>
                <div className="grid gap-1 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                    <SwitchRow
                        ...
                        checked={multiShot}
                        theme={theme}
                        onChange={(checked) => onMetadataChange?.(isKIEKlingV3 ? { multiShot: String(checked) } : { multiShot: String(checked), shotType: checked ? shotType : "intelligence" })}
                    />
                </div>
            </CanvasSettingGroup>
            ...
            <CanvasSettingGroup title="首尾帧" color={theme.node.muted}>
                <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                    <ResourceSinglePicker
                        ...
                        emptyText="暂无已连接图片"
                        theme={theme}
                        onChange={(value) => onMetadataChange?.({ klingImageNodeIds: [value, imageNodeIds[1]].filter(Boolean) })}
                    />
                    <ResourceSinglePicker
                        label="尾帧"
                        value={imageNodeIds[1] || ""}
                        options={imageOptions}
                        placeholder="不指定"
                        emptyText="暂无已连接图片"
                        theme={theme}
                        onChange={(value) => onMetadataChange?.({ klingImageNodeIds: [imageNodeIds[0], value].filter(Boolean) })}
                    />
                </div>
            </CanvasSettingGroup>
...
function FrameReferencePicker({ label, value, options, theme, onChange }: { label: string; value: string; options: CanvasVideoFrameOption[]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onChange: (value: string) => void }) {
    ...
    return (
        <div className="relative grid gap-1.5 text-xs" style={{ color: theme.node.muted }}>
            ...
            <button
                type="button"
                className="flex h-12 w-full min-w-0 max-w-full items-center gap-2 overflow-hidden rounded-xl border px-2 text-left transition hover:opacity-90"
                style={{ background: theme.node.fill, borderColor: open ? theme.toolbar.activeText : theme.node.stroke, color: theme.node.text }}
                onClick={() => setOpen((current) => !current)}
            >
                <FramePreview option={selected} />
                <span className="min-w-0 flex-1 overflow-hidden">
                    <span className="block truncate font-medium">{selected?.label || "不指定"}</span>
                    <span className="block truncate opacity-55">{selected ? "已连接图片节点" : options.length ? "点击选择已连接图片" : "暂无已连接图片"}</span>
                </span>
                {selected ? <ClearButton onClick={() => onChange("")} /> : null}
            </button>
            {open ? (
                <PickerMenu
                    items={items}
                    value={value}
                    theme={theme}
                    renderPreview={(item) => <FramePreview option={item.nodeId ? item : undefined} />}
                    renderTitle={(item) => item.label}
                    renderSubtitle={(item) => (item.nodeId ? "已连接图片节点" : "不使用首尾帧图片")}
                    onSelect={(nodeId) => {
                        onChange(nodeId);
                        setOpen(false);
                    }}
...
function ResourceSinglePicker({
    ...
    return (
        <div className="relative grid gap-1.5 text-xs" style={{ color: theme.node.muted }}>
            ...
            <button
                type="button"
                className="flex h-14 w-full min-w-0 max-w-full items-center gap-2 overflow-hidden rounded-xl border px-2 text-left transition hover:opacity-90"
                style={{ background: theme.node.fill, borderColor: open ? theme.toolbar.activeText : theme.node.stroke, color: theme.node.text }}
                onClick={() => setOpen((current) => !current)}
            >
...
function MultiResourcePicker({ values, options, theme, max = 4, onChange }: { values: string[]; options: CanvasVideoResourceOption[]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; max?: number; onChange: (values: string[]) => void }) {
    const [open, setOpen] = useState(false);
    ...
    const toggle = (nodeId: string) => {
        ...
        else if (values.length < max) onChange([...values, nodeId]);
    };
    return (
