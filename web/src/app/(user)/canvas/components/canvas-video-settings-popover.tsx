...
function H3ReferenceSettings({ metadata, resourceOptions, theme, onMetadataChange }: { metadata?: CanvasNodeMetadata; resourceOptions: CanvasVideoResourceOption[]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; onMetadataChange?: CanvasVideoSettingsPopoverProps["onMetadataChange"] }) {
        ...
        <>
            <CanvasSettingGroup title="H3 生成模式" color={theme.node.muted}>
                <div className="grid grid-cols-2 gap-2.5">
                    <OptionPill selected={!fullReference} theme={theme} onClick={() => onMetadataChange?.({ h3ReferenceMode: "frames" })}>首尾帧</OptionPill>
                    <OptionPill selected={fullReference} theme={theme} onClick={() => onMetadataChange?.({ h3ReferenceMode: "full" })}>全能参考</OptionPill>
                </div>
            </CanvasSettingGroup>
            {fullReference ? (
                <CanvasSettingGroup title="全能参考素材" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                        <p className="text-xs leading-5 opacity-65">关联图片、视频和音频节点后在此选择。未选时会使用全部关联素材；最多 9 张图片、3 个视频、3 段音频（总计 12 项）。提示词可用 &lt;Picture 1&gt;、&lt;Video 1&gt;、&lt;Audio 1&gt; 指定用途。</p>
                        <MultiResourcePicker values={selectedIds} options={resourceOptions.filter((item) => item.kind === "image" || item.kind === "video" || item.kind === "audio")} theme={theme} max={12} onChange={(nodeIds) => onMetadataChange?.({ h3ReferenceNodeIds: nodeIds })} />
                    </div>
                </CanvasSettingGroup>
            ) : (
                <CanvasSettingGroup title="首尾帧" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                        <FrameReferencePicker label="首帧" value={metadata?.firstFrameNodeId || ""} options={resourceOptions.filter((item) => item.kind === "image")} theme={theme} onChange={(value) => onMetadataChange?.({ firstFrameNodeId: value || undefined })} />
                        <FrameReferencePicker label="尾帧" value={metadata?.lastFrameNodeId || ""} options={resourceOptions.filter((item) => item.kind === "image")} theme={theme} onChange={(value) => onMetadataChange?.({ lastFrameNodeId: value || undefined })} />
                    </div>
                </CanvasSettingGroup>
            )}
        </>
