            ...
            <div ref={carouselRef} className="relative h-[calc((100vw-2rem)*.5625+56px)] w-screen overflow-hidden sm:h-[350px] sm:overflow-visible">
                {visibleBanners.map(({ banner, index, offset, position }) => {
                    ...
                    return (
                        ...
                        >
                            <span aria-hidden className={cn("pointer-events-none absolute inset-0 rounded-2xl", active ? "shadow-[0_18px_44px_rgba(0,0,0,0.5)]" : "shadow-[0_8px_20px_rgba(0,0,0,0.2)]")} />
                            <span className="relative isolate block size-full overflow-hidden rounded-2xl bg-stone-100 dark:bg-stone-900">
                ...
                <Button
                    type="text"
                    shape="circle"
                    className="!absolute !top-1/2 !z-10 !hidden !-translate-y-1/2 !bg-black/40 !text-white sm:!inline-flex"
            ...
            <div className="mt-1 flex items-center justify-center gap-2" aria-label="Banner 切换">
                {banners.map((banner, index) => (
                    <button
                        key={banner.imageUrl}
                        type="button"
                        className={cn("h-1.5 rounded-full transition-all", index === activeIndex ? "w-6 bg-stone-700 dark:bg-stone-200" : "w-2 bg-stone-300 dark:bg-stone-700")}
