    ...
    const readMediaFile = async (file?: File) => {
        ...
        const draft =
            formKind === "audio"
                ...
                : { url: media.url, storageKey: media.storageKey, width: media.width || 0, height: media.height || 0, bytes: media.bytes, mimeType: media.mimeType };
        setMediaDraft(draft);
                                ...
                                <div className="mt-2">
                                    {formKind === "image" && imageDraft ? (
                                        ...
                                        <Typography.Text type="secondary" className="text-xs">
                                            ...
                                            {typeof mediaDraft.bytes === "number" ? formatBytes(mediaDraft.bytes) : ""} · {mediaDraft.mimeType}
                                        </Typography.Text>
                                    ) : (
                                        <Typography.Text type="secondary" className="text-xs">
                                            未选择文件
                    ...
                    <div className="mt-3 overflow-hidden rounded-lg border border-stone-200 bg-background dark:border-stone-800">
                        {coverUrl || imageDraft?.dataUrl ? (
                            ...
                            <div className="flex aspect-[4/3] items-center justify-center bg-stone-100 p-5 dark:bg-stone-900">
                                <audio src={mediaDraft?.url || content} controls className="w-full" />
                            </div>
                        ) : (
